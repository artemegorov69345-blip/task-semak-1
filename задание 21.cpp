...
// Перечисляемый тип для операций
/**
* @brief Перечисяемый тип для выбора операции вычисления
* @details Определяет возможные операции, которые может выполнить программа
* @value P = 1 - Операция вычисления периметра треугольника
* @value S = 2 - Операция вычисления площади треугольника
*/
enum Operation {
    P = 1,
    S = 2
...
int main() {
   ...
   switch (w) {
        ...
        case S: {
            float result = Sv(x1, y1, x2, y2, x3, y3);
