...
int main() {
    ...
    double x = n;
    int t = 0;  // Переменная для подсчета точек
    
    cout << fixed << setprecision(4); // Устанавливаем точность вывода
    
    while (x <= k + h/1000) { // Добавляем небольшую погрешность
        t++;
        double y = vY(x);  // Используем правильное имя функции vY
    ...
    cout << "+-------------+-------------+" << endl;
    cout << "Всего вычислено точек: " << t << endl;  // Используем t вместо tochka
